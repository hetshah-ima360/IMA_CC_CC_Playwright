...
import rawData from '../../data/IMA_CC_CM_07.json';
...
/**
 * IMA_CC_CM_07 — create the Commitment Metrics contract, then read the newest
 * commitment number and run a Calculation Simulation, storing the result.
 * Expected results from the sheet are in data/IMA_CC_CM_07.json under "simulation".
 */
test.describe('Commitment Metrics — IMA_CC_CM_07 (Regression)', () => {
  test('IMA_CC_CM_07 - create contract then run simulation', async ({ page }) => {
    test.setTimeout(0);
    ...
    console.log(`>>> IMA_CC_CM_07: ${results.length} result row(s) captured`);
    results.forEach((r, i) => {
      const e = exp[i];
      console.log(
