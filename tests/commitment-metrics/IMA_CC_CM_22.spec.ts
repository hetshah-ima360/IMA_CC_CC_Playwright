...
import rawData from '../../data/IMA_CC_CM_22.json';
...
/**
 * IMA_CC_CM_22 — create the Commitment Metrics contract, then read the newest
 * commitment number and run a Calculation Simulation, storing the result.
 * Expected results from the sheet are in data/IMA_CC_CM_22.json under "simulation".
 */
test.describe('Commitment Metrics — IMA_CC_CM_22 (Regression)', () => {
  test('IMA_CC_CM_22 - create contract then run simulation', async ({ page }) => {
    test.setTimeout(0);
    ...
    console.log(`>>> IMA_CC_CM_22: ${results.length} result row(s) captured`);
    results.forEach((r, i) => {
      const e = exp[i];
      console.log(
